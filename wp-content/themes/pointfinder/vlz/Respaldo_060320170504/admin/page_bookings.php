    ...
    $this->FieldOutput .= '<div class="lista_reservas" data-user="'.$user_id.'">';
		...
		/*$sql = "
			SELECT 
				...
				reservas.post_parent AS orden,
				metas.post_id     	 AS ID,
				reservas.post_status AS status
			FROM 
				$wpdb->posts AS posts
			...
			LEFT JOIN $wpdb->posts    AS orden ON (orden.ID=reservas.post_parent AND orden.post_status != 'wc-pending')
			WHERE 
				posts.post_author = {$user_id} AND 
				posts.post_type   = 'product' AND
				(
					reservas.post_status != 'was-in-cart'
				)";*/
		...
		$sql = "
			...
			WHERE 
				...
				orden.post_status != 'wc-pending'  AND 
				...
				if( $reserva->status == "complete" ){
					...
					<tr>
						...
						<td style='text-align: center; width: 35px;'>
							<a class='btn_ver'
								href='".get_home_url()."/mi-cuenta/view-order/".($reserva->ID+1)."/'
								style='   
									font-weight: 600;
								    background: #54c8a7;
								    padding: 3px 5px;
					...
					$pendientes .= "
						<tr>
							...
							<td style='text-align: right; width: 194px;'>
								...
								>Ver</a>
								<a class='btn_ver'
									href='".get_home_url()."/wp-content/plugins/kmimos/order.php?o=".($reserva->ID+1)."&s=1'
									style='   
										font-weight: 600;
									    background: #54c8a7;
									    padding: 3px 5px;
								...
								<a class='btn_cancelar'
									href='".get_home_url()."/wp-content/plugins/kmimos/order.php?o=".($reserva->ID+1)."&s=0'
									style='   
										font-weight: 600;
									    background: #e80000;
									    padding: 3px 5px;
