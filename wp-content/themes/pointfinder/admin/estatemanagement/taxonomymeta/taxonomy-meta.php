...
class RW_Taxonomy_Meta {
	...
	function show( $tag, $taxonomy ) {
		...
		if ($meta_status == 'show') {
			...
			echo "<div class='pointfinder-tax-header'><h3>{$this->_meta['title']}</h3></div>
	...
	/******************** BEGIN META BOX FIELDS **********************/
		...
		function show_field_image( $field, $meta ) {
			...
			echo "<a href='#' class='rwtm-image-upload button' data-field='{$field['id']}'>" . esc_html__( 'Select Image', 'pointfindert2d' ) . "</a>";
			echo '</td>';
		}
