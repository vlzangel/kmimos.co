...
// Exit if accessed directly
    ...
    if ( ! class_exists( 'ReduxFramework_Extension_custom_sorter' ) ) {
        class ReduxFramework_Extension_custom_sorter {
                        ...
                        foreach ( $sortlists as $group => $sortlist ) {
                            ...
                            foreach ( $sortlist as $key => $list ) {
                                ...
                                if ( $key != "placebo" ) {
                                    ...
                                    echo '<input class="position ' . $this->field['class'] . ''. $list['clstype'] .'" type="hidden" name="' . $this->field['name'] . '[' . $group . '][' . $key . ']' . $this->field['name_suffix'] . '" value="' . $list['name']. '">';
                                    
                                    switch ($list['clstype']) {
                                        case 'pfsingle':
                                            echo '<div class="pfclassic">'.$list['name'].'</div>';
                                            break;

                                        case 'pfdouble':
                                            $exploded_attr = explode('&', $list['name']);
                                            echo '<div class="pfdouble pfinnersorter"><span>'.$exploded_attr[0].'</span></div>';
                                            echo '<div class="pfdouble pfinnersorter"><span>'.$exploded_attr[1].'</span></div>';
                                            break;
                                        case 'pftriple1':
                                            $exploded_attr = explode('&', $list['name']);
                                            $exploded_attr2 = explode('+', $exploded_attr[0]);
                                            echo '<div class="pftriple1 pfinnersorter">';
                                            echo '<span class="pftripleinner">'.$exploded_attr2[0].'</span><span class="pftripleinner">'.$exploded_attr2[1].'</span>';
                                            echo '</div>';
                                            echo '<div class="pftriple1 pfinnersorter"><span>'.$exploded_attr[1].'</span></div>';
                                            break;
                                        case 'pftriple2':
                                            $exploded_attr = explode('&', $list['name']);
                                            $exploded_attr2 = explode('+', $exploded_attr[1]);
                                            echo '<div class="pftriple2 pfinnersorter"><span>'.$exploded_attr[0].'</span></div>';
                                            echo '<div class="pftriple2 pfinnersorter">';
                                            echo '<span class="pftripleinner">'.$exploded_attr2[0].'</span><span class="pftripleinner">'.$exploded_attr2[1].'</span>';
                                            echo '</div>';
