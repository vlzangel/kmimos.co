...
abstract class BaseFacebook
{
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    */
    ...
    print_r($ch);
    echo 'params';
    wp_die(print_r(http_build_query($params, null, '&')));
    */
    ...
    */
      ...
      if(isset($response['response']['code'])){
        if ($response['response']['code'] != 200) {
          wp_die(sprintf(esc_html__('An error occured on request %s','pointfindert2d'),'Code1'));
        }else{
      ...
      if (is_wp_error($response)) {
        wp_die(sprintf(esc_html__('An error occured on request %s','pointfindert2d'),'Code2'));
      }

    }else{
      wp_die(sprintf(esc_html__('An error occured on request %s','pointfindert2d'),'Code3'));
    }
  }
