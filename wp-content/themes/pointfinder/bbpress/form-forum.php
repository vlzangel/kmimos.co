			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( __( 'Now Editing &ldquo;%s&rdquo;', 'pointfindert2d' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( __( 'Create New Forum in &ldquo;%s&rdquo;', 'pointfindert2d' ), bbp_get_forum_title() ) : _e( 'Create New Forum', 'pointfindert2d' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php _e( 'This forum is closed to new content, however your account still allows you to do so.', 'pointfindert2d' ); ?></p>
					</div>
				...
				<?php if ( current_user_can( 'unfiltered_html' ) ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php _e( 'Your account has the ability to post unrestricted HTML content.', 'pointfindert2d' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( __( 'Forum Name (Maximum Length: %d):', 'pointfindert2d' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<?php if ( ! ( bbp_use_wp_editor() || current_user_can( 'unfiltered_html' ) ) ) : ?>
						...
						<p class="form-allowed-tags">
							<label><?php _e( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes:','pointfindert2d' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php _e( 'Forum Type:', 'pointfindert2d' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php _e( 'Status:', 'pointfindert2d' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php _e( 'Visibility:', 'pointfindert2d' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php _e( 'Parent Forum:', 'pointfindert2d' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => __( '(No Parent)', 'pointfindert2d' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php _e( 'Submit', 'pointfindert2d' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( __( 'The forum &#8216;%s&#8217; is closed to new content.', 'pointfindert2d' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? _e( 'You cannot create new forums.', 'pointfindert2d' ) : _e( 'You must be logged in to create new forums.', 'pointfindert2d' ); ?></p>
		</div>
	</div>
