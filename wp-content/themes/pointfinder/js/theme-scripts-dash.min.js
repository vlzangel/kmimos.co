(function($){"use strict";$.pf_submit_page_map=function(){var mapcontainer=$('#pfupload_map');var pf_lat=mapcontainer.data('pf-lat');var pf_lng=mapcontainer.data('pf-lng');var pf_type=mapcontainer.data('pf-type');var pf_zoom=mapcontainer.data('pf-zoom');mapcontainer.gmap3({map:{options:{center:[pf_lat,pf_lng],zoom:pf_zoom,mapTypeId:google.maps.MapTypeId.pf_type,mapTypeControl:true,mapTypeControlOptions:{style:google.maps.MapTypeControlStyle.DROPDOWN_MENU,position:google.maps.ControlPosition.RIGHT_BOTTOM},zoomControl:true,zoomControlOptions:{position:google.maps.ControlPosition.LEFT_BOTTOM},panControl:false,scaleControl:false,navigationControl:false,draggable:true,scrollwheel:false,streetViewControl:false,},callback:function(){setTimeout(function(){$.pf_submit_page_map_fallback();},800);}},marker:{values:[{latLng:[pf_lat,pf_lng],}],options:{draggable:true},events:{dragend:function(marker){$('#pfupload_lat_coordinate').val(marker.getPosition().lat());$('#pfupload_lng_coordinate').val(marker.getPosition().lng());},},}});};$.pf_submit_page_map_fallback=function(){if($(".pf-search-locatemebut").length){$(".pf-search-locatemebut").svgInject();};var pf_map=$('#pfupload_map').gmap3("get");var pf_input=document.getElementById("pfupload_address");$("#pfupload_address").bind("keypress",function(e){if(e.keyCode==13){e.preventDefault();return false;}});var autocomplete=new google.maps.places.Autocomplete(pf_input);autocomplete.bindTo("bounds",pf_map);google.maps.event.addListener(autocomplete,"place_changed",function(){var place=autocomplete.getPlace();if(!place.geometry){return;}
...
var marker=$('#pfupload_map').gmap3({get:"marker"});marker.setPosition(place.geometry.location);$("#pfupload_lat_coordinate").val(marker.getPosition().lat());$("#pfupload_lng_coordinate").val(marker.getPosition().lng());});}
...
if(modalname=='creditcardstripe'){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show',message:theme_scriptspfm.paypalredirect2});}else if(modalname=='paypalrequest'){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show',message:theme_scriptspfm.paypalredirect});}else if(modalname=='stripepayment'){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'show',message:theme_scriptspfm.paypalredirect3});};var minwidthofdialog=380;if(!$.pf_mobile_check()){minwidthofdialog=320;};$.ajax({type:'POST',dataType:'json',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_paymentsystem','formtype':modalname,'itemid':itemid,'otype':otype,'token':token,'security':theme_scriptspfm.pfget_paymentsystem},success:function(data){var obj=[];$.each(data,function(index,element){obj[index]=element;});if(obj.process==true){if(modalname=='paypalrequest'){window.location=obj.returnurl;}else if(modalname=='creditcardstripe'){var handler=StripeCheckout.configure({key:obj.key,token:function(token){$.pfOpenPaymentModal('open','stripepayment',itemid,token,obj.otype);}});handler.open({name:obj.name,description:obj.description,amount:obj.amount,email:obj.email,currency:obj.currency,allowRememberMe:false,opened:function(){$.pfOpenPaymentModal('close');},closed:function(){if($('#pfupload_type').val()==1){window.location=theme_scriptspfm.dashurl2;};}});$(window).on('popstate',function(){handler.close();});}else if(modalname=='stripepayment'){$('#pf-membersystem-dialog').pfLoadingOverlay({action:'hide'});$("#pf-membersystem-dialog").html(obj.mes);var pfreviewoverlay=$("#pfmdcontainer-overlay");pfreviewoverlay.show("slide",{direction:"up"},100);window.location=obj.returnurl;};}else{$('#pf-membersystem-dialog').pfLoadingOverlay({action:'hide'});$("#pf-membersystem-dialog").html(obj.mes);var pfreviewoverlay=$("#pfmdcontainer-overlay");pfreviewoverlay.show("slide",{direction:"up"},100);}
...
return false;});$(".pfstatusbuttonaction").on('click touchstart','',function(event){var pid=$(this).data('pfid');var thisitem=$(this);$.ajax({beforeSend:function(){$('.pfmu-itemlisting-inner'+pid).pfLoadingOverlay({action:'show'});},type:'POST',dataType:'json',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_onoffsystem','itemid':pid,'lang':theme_scriptspfm.pfcurlang,'security':theme_scriptspfm.pfget_onoffsystem}}).done(function(obj){if(obj==1){thisitem.switchClass('pfstatusbuttonactive','pfstatusbuttondeactive');}else{thisitem.switchClass('pfstatusbuttondeactive','pfstatusbuttonactive');};$('.pfmu-itemlisting-inner'+pid).pfLoadingOverlay({action:'hide'});})});})(jQuery);